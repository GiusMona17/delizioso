...
class CookistSearchProvider(
    ...
    fun parseSearchResults(html: String): List<OnlineSearchResult> {
        val doc = Jsoup.parse(html, "https://www.cookist.it/")
        val articles = doc.select("article, .card, .post-item, .item-recipe, .cookist-card")
        ...
        for (article in articles) {
            ...
            if (href.isEmpty() || !href.contains("cookist.it") || href == "https://www.cookist.it/") {
                continue
            }
            if (seenUrls.contains(href)) continue
            seenUrls.add(href)

            val titleEl = article.selectFirst("h2, h3, h4, .title, .card-title") ?: continue
